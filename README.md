...
- 点击任务文本打开对应任务笔记。
- 拖动任意一张任务卡片，卡片会被「举起」浮在桌面上方，列表底部同时弹出投放区，包含「完成 / 恢复进行中」和「归档」两个圆形目标。
  - 把卡片丢进「完成」即可标记完成；如果当前任务已是完成状态，目标会自动切换为「恢复进行中」，丢进去就回到进行中。
  - 把卡片丢进「归档」即可归档，归档后只能在「归档管理」里找到。
...
点击 macOS 菜单栏中的 TaskSnap 图标，右键打开菜单后选择「归档管理…」，可以进入归档任务页面：

- 左侧按归档时间展示所有归档任务的标题和创建时间。
- 右侧展示选中归档任务的详细信息：描述、来源、截图大图以及对应的 Markdown 笔记预览。
- 顶部的「撤销归档」按钮会把当前任务恢复为进行中，并放回主任务列表。
